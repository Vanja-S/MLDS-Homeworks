...
1. The importance function was correct and mathematically sound, but the report I turned in didn't correctly display the graph or the usage (the comparison) it should've shown a comparison between two methods (to some baseline) using the importance function.
2. The BetterTree approached were valid, but yielded almost zero improvemnt -> I did get extra points for this, but not enought to counter the previous two problems.
